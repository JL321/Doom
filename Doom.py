...
for i in range(replay_min):
    ...
    if done:
        ...
        experience = state, action, next_state, done
...
def update_target_graph():
    ...
    from_vars = tf.get_collection(tf.GraphKey.TRAINABLE_VARIABLES, "DQN")
    
    to_vars = tf.collection(tf.GraphKeys.TRAINABLE_VARIABLES, "TDQN")
    ...
    with tf.Session() as sess:
        ...
        for episode in range(total_episodes):
            ...
            while step < max_steps:
                ...
                q_target_next_state = sess.run(TargetNet.output, feed_dict = {TargetNet.inputs_: next_states_mb})
